...
/**
 * Score candidate exercise matches against target exercise name to find the best match.
 * Enforces 100% strict match accuracy and prevents matching wrong exercises (e.g. Arnold Press for Leg Press).
 */
...
/**
 * Clean AI or raw exercise titles for search (e.g. "AI Hypertrophy Bench Press" -> "Bench Press")
 */
...
/**
 * Fetch rich exercise details (videos, images, instructions) from ExerciseDB V2 on RapidAPI
 * Uses fuzzy search endpoint + relevance scoring + strict score thresholding + 5-key rotation pool.
 */
export async function fetchExerciseDbDetails(exerciseName: string): Promise<ExerciseDbResult> {
      ...
      try {
        ...
        if (searchRes.ok) {
          ...
          if (Array.isArray(results) && results.length > 0) {
            // Rank candidates using smart matching score
            ...
            // STRICT THRESHOLD CHECK: If score < 250, reject match to prevent showing wrong video
            ...
            // Fetch full exercise detail by exerciseId to get videoUrl, overview, instructions, & tips if needed
            ...
            // Title-case target muscle string
  ...
  // Fallback defaults if API search misses, matches fail threshold, or keys are unavailable
  return {
    name: exerciseName,
    videoUrl: '',
    imageUrl: '',
